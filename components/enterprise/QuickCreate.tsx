...
const quickActions = [
  { label: "New Customer", icon: Users, href: "/customers", description: "Add a new customer record" },
  ...
  { label: "New Lead", icon: Target, href: "/leads", description: "Capture a new sales lead" },
    ...
    <DialogPrimitive.Root open={open} onOpenChange={(v) => { if (!v) onClose(); }}>
      <DialogPrimitive.Portal>
        ...
        <DialogPrimitive.Popup className="fixed inset-0 z-50 flex items-start justify-center pt-[20vh] data-ending-style:opacity-0 data-starting-style:opacity-0 data-ending-style:scale-95 data-starting-style:scale-95 transition-all duration-150">
          <div className="w-full max-w-md rounded-xl border bg-white p-6 shadow-2xl dark:bg-slate-950 dark:border-slate-800">
              ...
              {quickActions.map((action) => {
                ...
                return (
                  <button
                    ...
                    className="flex items-center gap-3 rounded-lg p-3 text-left transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/50"
                  >
